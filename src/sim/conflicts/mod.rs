...
fn muster_armies(ctx: &mut TickContext, time: SimTimestamp, current_year: u32) {
    ...
    for faction_id in at_war_factions {
            ...
            helpers::faction_capital_largest(ctx.world, faction_id)
        {
            ...
            ctx.world
                .set_extra(army_id, K::HOME_REGION_ID, serde_json::json!(region_id), ev);
        }
        ctx.world.set_extra(
            army_id,
            K::STARTING_STRENGTH,
        ...
        ctx.world
            .set_extra(army_id, K::MONTHS_CAMPAIGNING, serde_json::json!(0u32), ev);
...
fn apply_supply_and_attrition(ctx: &mut TickContext, time: SimTimestamp, current_year: u32) {
    let armies: Vec<(u64, u64)> = ctx
        ...
        .map(|e| {
            let faction_id = e.extra_u64_or(K::FACTION_ID, 0);
            (e.id, faction_id)
        })
        .collect();
    ...
    for (army_id, faction_id) in armies {
        ...
        let is_besieging = ctx
            ...
            .entities
            .get(&army_id)
            .is_some_and(|e| e.extra.contains_key(K::BESIEGING_SETTLEMENT_ID));
        ...
        let home_region = ctx
            ...
            .entities
            .get(&army_id)
            .and_then(|e| e.extra_u64(K::HOME_REGION_ID));
        ...
        let months = ctx
            ...
            .entities
            .get(&army_id)
            .map(|e| e.extra_u64_or(K::MONTHS_CAMPAIGNING, 0))
            .unwrap_or(0) as u32;
...
fn check_retreats(ctx: &mut TickContext, time: SimTimestamp, current_year: u32) {
    let armies: Vec<(u64, u64, Option<u64>)> = ctx
        ...
        .filter(|e| e.kind == EntityKind::Army && e.end.is_none())
        .map(|e| {
            let home = e.extra_u64(K::HOME_REGION_ID);
            let starting = e.extra_u64_or(K::STARTING_STRENGTH, 1) as u32;
    ...
    for (army_id, starting_strength, home_region) in armies {
        ...
        let besieging = ctx
            ...
            .entities
            .get(&army_id)
            .and_then(|e| e.extra_u64(K::BESIEGING_SETTLEMENT_ID));
        if let Some(siege_settlement_id) = besieging {
            ...
            let attacker_faction = ctx
                ...
                .entities
                .get(&army_id)
                .map(|e| e.extra_u64_or(K::FACTION_ID, 0))
                .unwrap_or(0);
            siege::clear_siege(
                ctx,
...
mod tests {
    ...
    fn assault_success_with_overwhelming_force() {
        ...
        for seed in 0..100 {
            ...
            w.set_extra(a, K::BESIEGING_SETTLEMENT_ID, serde_json::json!(s), 1);
