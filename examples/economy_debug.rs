...
fn main() {
    ...
    for e in world.entities.values() {
        if e.kind == EntityKind::Faction && e.end.is_none() {
            ...
            let happiness = fd.map(|f| f.happiness).unwrap_or(-1.0);
            let stability = fd.map(|f| f.stability).unwrap_or(-1.0);
            let treasury = fd.map(|f| f.treasury).unwrap_or(-1.0);
            let econ_motivation = e
                .extra
                .get("economic_war_motivation")
                .and_then(|v| v.as_f64())
                .unwrap_or(0.0);
            let has_enemies = e
                .relationships
            ...
            eprintln!(
                "Faction {} ({}): happiness={:.3} stability={:.3} treasury={:.1} econ_motivation={:.3} enemies={} allies={}",
                e.id,
                e.name,
                happiness,
                stability,
    ...
    for e in world.entities.values() {
        if e.kind == EntityKind::Settlement && e.end.is_none() {
            let prosperity = e.data.as_settlement().map(|s| s.prosperity).unwrap_or(-1.0);
            prosperities.push(prosperity);
        }
    }
    eprintln!(
        "Settlement prosperities: min={:.3} max={:.3} avg={:.3}",
    ...
    let wars = world
        .events
        .values()
        .filter(|e| e.kind == history_gen::model::EventKind::WarDeclared)
        .count();
    let battles = world
        .events
        .values()
        .filter(|e| e.kind == history_gen::model::EventKind::Battle)
        .count();
